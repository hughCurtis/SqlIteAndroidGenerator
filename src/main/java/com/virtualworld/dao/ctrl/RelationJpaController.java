...
public class RelationJpaController implements Serializable {
    ...
    private static final Logger LOG = Logger.getLogger(RelationJpaController.class.getName());
    ...
    public List<Relation> findByDbId(Short idDb) {
        EntityManager em = getEntityManager();
        try {
            Query q = em.createQuery("SELECT r FROM Relation r WHERE r.dataBase.id = ?1");
            q.setParameter(1, idDb);
            return q.getResultList();
        } catch (Exception e) {
